...
import styles from './LuceDiTerraShop.module.css';

// Identical scroll-driven GSAP logic to ShopSlider.tsx — only images, product copy,
// panel colors and the badge label differ. See ShopSlider.tsx for the annotated
// history of the cutout-transition approach (Flip was tried and reverted there).
gsap.registerPlugin(ScrollTrigger);

interface LuceDiTerraShopProps {
  prodotti: Prodotto[];
}
  ...
  return (
    <article className={`${styles.mobileCard} ${styles[BG_CLASS[p.coloreSfondo]]}`}>
      ...
      <div className={styles.mobileBody}>
        <span className={styles.badge}>Luce di Terra</span>
...
export default function LuceDiTerraShop({ prodotti }: LuceDiTerraShopProps) {
  ...
  return (
    <>
      ...
      >
        ...
        <div ref={sliderLeftRef} className={styles.sliderLeft}>
          ...
          {prodotti.map((p, i) => {
            ...
            return (
              ...
              >
                {/* Upper 2/3 — dark Luce di Terra background, hosts the cutout image */}
          ...
          {/* Cutout images - siblings of panels, animate independently.
              NOTE: no mix-blend-mode here. These PNGs already have a real alpha
              channel from AI background removal (see scripts/remove-background-ai.js) —
              they never had a white/gray backdrop baked in to begin with. Adding
              mix-blend-mode: multiply against the dark #1A1208 leftTop background
              was tested and made every jar render almost black (multiply darkens
              toward the blend-mode partner's own color, which is very dark here) —
              see the composited test output during development. Plain alpha
              compositing is the correct approach for an already-transparent PNG. */}
          {prodotti.map((p, i) =>
            p.cutoutImage ? (
              <div
                key={`cutout-${p.id}`}
          ...
          {prodotti.map((p, i) => (
            ...
            >
              <div className={styles.rightInner}>
                ...
                {/* Badge — fixed "Luce di Terra" label for every product in this shop */}
                <span
                ...
                >
                  {p.nome.split(' ').map((word, wi, words) => (
                    // The space is a sibling text node, not content inside the
                    // inline-block span — a trailing space *inside* an
                    // inline-block gets collapsed by the browser at layout time,
                    // which is what glued the words together with no gap.
                    <span key={wi}>
        ...
        <div className={styles.progressRail} aria-label="Naviga prodotti Luce di Terra">
          <button
      ...
      {/* ── Mobile ── */}
      <section className={styles.mobileList} aria-label="Shop Luce di Terra">
        {prodotti.map((p, i) => (
